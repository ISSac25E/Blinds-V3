...
#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\Serge\Core\StepperDriver\StepperDriver_2.1.3.h"
#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\Serge\Core\StepperQueue\StepperQueue_1.0.0.h"
...
void checkUpdate()
{
  // check if anny stepper are currently moving. If so, update their position:
  if (currentStepperProccessID >= 0)
  {
    // update position:
    tuya_module.mcu_dp_update(StepperConfigObj[currentStepperProccessID].position_DPID, map(stepperCurrentPos[currentStepperProccessID], 0, StepperConfigObj[currentStepperProccessID].stepperMaxStep, 0, 100), 1);
